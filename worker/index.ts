...
const STALE_ID_PATTERN = /does not exist|not found|invalid.*inventory ?item|could not find/i;

function log(message: string): void {
  console.log(`[${new Date().toISOString()}] ${message}`);
}

function hasStaleIdFailure(entries: { status: string; message?: string }[]): boolean {
  return entries.some((e) => e.status === "failed" && e.message && STALE_ID_PATTERN.test(e.message));
}
...
async function runSync(): Promise<void> {
  ...
  if (applyResult.pullReport.stoppedEarly) {
    log(`sync: HALTED — dead Kelme token (${applyResult.pullReport.stopReason}). ` +
      `No Shopify write attempted. Manual re-auth required before the next sync can proceed.`);
    return;
  }

  if (applyResult.halted) {
    log(`sync: HALTED — sanity bound tripped (${applyResult.haltReason}). No Shopify write attempted.`);
    return;
  }

  log(`sync: DB caught up (matched=${applyResult.matched} updated=${applyResult.updated} ` +
    `unmatched=${applyResult.unmatched.length}) — proceeding to Shopify push`);

  let pushReport = await executePush();
  log(`sync: push ok=${pushReport.ok} failed=${pushReport.failed} ` +
  ...
  if (pushReport.failed > 0 && hasStaleIdFailure(pushReport.entries)) {
    log("sync: stale inventory item ID detected in push failures — relinking and retrying once");
    try {
      const relinkResult = await executeRelink();
      log(`sync: relink complete (updated=${relinkResult.updated}) — retrying push`);
      pushReport = await executePush();
      log(`sync: retry push ok=${pushReport.ok} failed=${pushReport.failed}`);
    } catch (err) {
      log(`sync: relink/retry failed: ${err instanceof Error ? err.message : String(err)}`);
    }
  }

  const verifyResult = await verifyPush();
  log(`sync: verify totalChecked=${verifyResult.totalChecked} correct=${verifyResult.correct} ` +
    `mismatched=${verifyResult.mismatched.length}`);
  if (verifyResult.mismatched.length > 0) {
    log(`sync: verify mismatches: ${JSON.stringify(verifyResult.mismatched.slice(0, 10))}`);
  }
...
async function runSyncSafely(): Promise<void> {
  try {
    ...
    log(`sync: FAILED with unexpected error: ${err instanceof Error ? err.stack ?? err.message : String(err)}`);
  }
}
